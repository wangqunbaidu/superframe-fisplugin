...
class FISResource {
    ...
    //输出模板的最后，替换css hook为css标签集合,替换js hook为js代码
    public static function rendersfResponse($strContent){
        $cssIntPos = strpos($strContent, self::CSS_LINKS_HOOK);
        if($cssIntPos !== false){
            $strContent = substr_replace($strContent, self::render('css'), $cssIntPos, strlen(self::CSS_LINKS_HOOK));
        }
        $frameworkIntPos = strpos($strContent, '<JS_FRAMEWORK_HOOK/>');
        $sfmatches ='';
        if($frameworkIntPos !== false){
            $jssfframe = self::getModJsHtml();
            preg_match('/(<script type="text\/javascript">[^>]+<\/script>)/', $jssfframe, $sfmatches);
            $jssfframe = preg_replace('/(<script type="text\/javascript">[^>]+<\/script>)/', '', $jssfframe);

            $JsLink = $jssfframe. self::render('js');
            if($sfmatches[0]){
                $sfmatches = $sfmatches[0];
                $sfmatches = preg_replace('/(<script type="text\/javascript">)/', '<script>', $sfmatches);
            }


            $strContent = substr_replace($strContent, $JsLink, $frameworkIntPos, strlen('<JS_FRAMEWORK_HOOK/>'));
        }
        ...
        if($jsIntPos !== false){
           // $jsContent = ($frameworkIntPos !== false) ? '' : self::getModJsHtml();
            ...
            if($sfmatches){
                ...
                $jsContent =  $sfmatches . self::renderSFScriptPool();
            }
    ...
    private static function getModJsHtml(){
        ...
        if ($resourceMap) {
            ...
            //fix esl
            $html .= 'require.siteNS && require.siteNS(["'.implode('", "', array_keys(self::$arrMap)).'"]);';
    ...
    //输出js，将页面的js源代码集合到pool，一起输出
    public static function renderSFScriptPool(){
        $html = '';
        if(!empty(self::$arrScriptPool)) {
            $priorities =  array_keys(self::$arrScriptPool);
            rsort($priorities);
            foreach ($priorities as $priority) {
                $html .= '<script>!function(){' . implode("}();\n!function(){", self::$arrScriptPool[$priority]) . '}();</script>';
            }
        }
        return $html;
    }
